...
def run_cmd(cmd: list[str], cwd: Path = ROOT, timeout: int = 300) -> tuple[int, str, str]:
    ...
    # Ensure .venv/bin and ~/.local/bin are in PATH for pytest and omp
    venv_bin = str(ROOT / ".venv" / "bin")
    local_bin = str(Path.home() / ".local" / "bin")
    for p in (venv_bin, local_bin):
...
def _omp_coding(task: str, branch: str) -> tuple[bool, str]:
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = str(ROOT)
    # Resolve omp binary — may be omp.sh or omp depending on install method
    omp_bin = str(Path.home() / ".local" / "bin" / "omp")
    rc, out, err = run_cmd(
        ["bash", "-c", f"cd {ROOT} && git checkout -b {branch} 2>/dev/null || git checkout {branch} && {omp_bin} < {prompt_file}"],
        timeout=600,
    )
